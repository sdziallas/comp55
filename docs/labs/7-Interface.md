...
While you can write this separately, an easier way to do this is to go to your ```EmotionalCharacter``` and in you ```makeImageCharacter```, where you create your ```EmotionalCharacter```, add in the paranthesis of that constructor the name of your local variable for your GImage.  You should at that point get a red squiggly under it.  If you then do a quick fix and choose create constructor, eclipse will then create a constructor for you in ```EmotionalCharacter``` that will have a ```GImage``` as a parameter.  Then in that constructor make sure add in a line that would set the parameter in to your instance variable, similar to what you did in your [early java lab](2-JavaIntro.html)
...
#### ***Run the program***
You should now see your three objects on the screen, and when you click on the three methods.  Assuming you typed the names correctly for each of the three files, then you will see the EmotionalCharacter cycle through its states.  Now we have three different objects that are all different, and yet all can end up being Emotional and thus express the three emotions that we have outlined.  If you want to see what the final program should look like, then you can [click here](lab7media/media/)
### Step 6: Make your own class!
While all of what we have done has felt very similar,  Remember that with an interface, you have utmost flexibility!  All you have to do is create a class that ```implements Emotional```, and as long as you define those three methods, then you can participate in the EmotionalSimulator (as long as you create that object and add yourself to the list of ```individuals```).  Heck, you could even have something that ***isn't even graphical at all***, it could just print out statements to the console when those methods are called.  You have complete control as to what you would like to do.  So for the rest of the lab, I would like you to create a new class and add it to the simulator, so that when it is called, you see that you have something that implements the three methods.  Once you are done, you will take three screenshots of the three states, showing the four different reactions from the different classes that you have created and upload all three images them as a zip, that show three different states that are reminiscent of these three states.  